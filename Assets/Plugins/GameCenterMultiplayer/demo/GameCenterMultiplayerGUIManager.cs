...
public class GameCenterMultiplayerGUIManager : MonoBehaviourGUI
{
	...
	void Start()
	{
		GameCenterManager.playerAuthenticated += () => { Debug.Log( "Player authenticated" ); };
	...
	void OnGUI()
	{
		...
		{
			var error = GameCenterMultiplayerBinding.sendMessageToAllPeers( "GCTestObject", "receiveTime", Time.timeSinceLevelLoad.ToString(), false );
			if( error != null )
